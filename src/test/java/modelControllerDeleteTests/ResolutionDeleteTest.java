...
public class ResolutionDeleteTest {
    ...
    public void setUp() throws Exception {
        ...
        formDataController.saveDataFromResolutionForm(0 + "", table1);
        formController.createTableItem(SegmentType.Resolution);
        formDataController.saveDataFromResolutionForm(0 + "", new ClassTable("","","",true, 1));
        ...
        mapperTableToObject.mapTableToWU(indicators, indicators, indicators, indicators, indicators, indicators
                ,indicators, 0,"Test");
        deleteFormController.deleteResolution(list, branchTables);

    }

    @Test
    public void testMapper() {
        Set instacies =  mapperTableToObject.getWUToResolutionMapper().keySet();
        assertSame(1, instacies.size());
        assertSame(1, instacies.iterator().next());
    }
    @Test
    public void testListSize() {
        assertSame(1, dataModel.getResolutions().size());
    }

    @Test
    public void testDeleteCorectItem() {
        assertSame(1, dataModel.getResolutions().get(0).getId());
    }
    ...
    @Test
    public void testWU() {
        assertSame(1, workUnit.getResolutionIndex().size());
        int index = workUnit.getResolutionIndex().get(0);
        assertSame(1, dataModel.getResolutionId(index));
    }
}